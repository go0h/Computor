...
class PolyOptimizeTest extends AnyFunSuite {
  ...
  test("Optimize - 1") {
    ...
    val opt = Polynomial(expr).toString
    assert(opt.equals("-1.0 * X^12.0 + X^3.23 + 4.0 * X^3.0 + 10.0 = 0.0"), opt)
  }
  ...
  test("Unary Realnumber - 1") {
    val expr = "4 - -8 * X^4 / (4 * X^2) * (-2 * X^1) + X^3.23 = -4 - 10 + X^12"
    val opt = Polynomial(expr).toString
    assert(opt.equals("-1.0 * X^12.0 + X^3.23 + -4.0 * X^3.0 + 18.0 = 0.0"), opt)
  }
  ...
  test("Optimize - 7") {
    ...
    val opt = Polynomial(expr).toString
    assert(opt.equals("X^16.0 + X + 10.0 = 0.0"), opt)
  }
  ...
  test("Subject test - 1") {
    ...
    assert(norm.equals("-9.3 * X^2.0 + 4.0 * X + 4.0 = 0.0"), norm)
  }

  test("Subject test - 2") {
    val expr = "5 * X^0 + 4 * X^1 = 4 * X^0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("4.0 * X + 1.0 = 0.0"), norm)
  }

  test("Subject test - 3") {
    val expr = "8 * X^0 - 6 * X^1 + 0 * X^2 - 5.6 * X^3 = 3 * X^0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("-5.6 * X^3.0 + -6.0 * X + 5.0 = 0.0"), norm)
  }

  test("Subject test - 4") {
    val expr = "5 + 4 * X + X^2= X^2"
    val norm = Polynomial(expr).toString
    assert(norm.equals("4.0 * X + 5.0 = 0.0"), norm)
  }

  test("Negative braces - 1") {
    val expr =   "5 * X^1 - ((4 * X^1 + 4) + 4) = 0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("X + -8.0 = 0.0"), norm)
  }

  test("Negative braces - 2") {
    val expr =   "5 * X^1 - (4 * X^1 + 4) + 4 = 0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("X = 0.0"), norm)
  }

  test("Negative braces - 3") {
    val expr =   "-(5 * X^1) - (4 * X^1 + 4) + 4 = 0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("-9.0 * X = 0.0"), norm)
  }

  test("Negative braces - 4") {
    val expr =   "-((5 * X^1) - ((4 * X^1 + 4) + 4)) = 0"
    val norm = Polynomial(expr).toString
    assert(norm.equals("-1.0 * X + 8.0 = 0.0"), norm)
  }

  test("Negative variable") {
    val expr = "8 * y^0.12 * -4 * y^1 + y^1 + 4 * -y^3 + 31 = 0"
    val opt = Polynomial(expr).toString
    assert(opt.equals("-4.0 * y^3.0 + -32.0 * y^1.12 + y + 31.0 = 0.0"), opt)
  }
}
