...
class PolyStringTransformTest extends AnyFunSuite {
  ...
  def normalizePolynomial(expr: String): String = {
    ...
    val leftExpr = toOptimalExpression(expr.substring(0, expr.indexOf("="))).simplify
    val rightExpr = toOptimalExpression(expr.substring(expr.indexOf("=") + 1)).simplify
  ...
  test("Basic convention - 3") {
    ...
    val norm = normalizePolynomial(expr)
    assert(norm.equals("(4.0 * X^2.0) + (3.0 * X^1.0) + 5.0 = 123.43"), norm)
  }
  ...
  test("Basic convention - 7") {
    val expr = "X ^ 4 = 77"
    val norm = normalizePolynomial(expr)
    assert(norm.equals("(1.0 * X^4.0) = 77.0"), norm)
  }

  test("Basic convention - 8") {
    val expr = "X * 4 = 77"
    val norm = normalizePolynomial(expr)
    assert(norm.equals("(4.0 * X^1.0) = 77.0"), norm)
  }
  ...
  test("Division convention - 4") {
    val expr = "X / 4 = 77"
    val norm = normalizePolynomial(expr)
    assert(norm.equals("(0.25 * X^1.0) = 77.0"), norm)
  }
  ...
  test("Subject test - 2") {
    ...
    val norm = normalizePolynomial(expr)
    assert(norm.equals("5.0 + (4.0 * X^1.0) = 4.0"), norm)
  }
  ...
  test("Subject test - 4") {
    ...
    val norm = normalizePolynomial(expr)
    assert(norm.equals("5.0 + (4.0 * X^1.0) + (1.0 * X^2.0) = (1.0 * X^2.0)"), norm)
  }
